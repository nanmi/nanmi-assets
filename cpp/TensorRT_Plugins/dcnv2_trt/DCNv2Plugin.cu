...
#define cublasCheck(op) \
 	do {	\
 		...
 		if (ret != CUBLAS_STATUS_SUCCESS) {	\
 			printf("%s fail, %d != %d", #op, ret, CUBLAS_STATUS_SUCCESS); \
 			abort();	\
 		}	\
 	} while (0);
    ...
    cudaStream_t stream)
{
	...
	size_t maskSize = static_cast<size_t>(input1_shape.d[H_DIM] * input1_shape.d[W_DIM] * kernel_size * kernel_size * deformable_groups);
	size_t im2colSize = static_cast<size_t>(input1_shape.d[C_DIM] * kernel_size * kernel_size * output_shape.d[H_DIM] * output_shape.d[W_DIM]);
	...
	for (int ibatch = 0 ; ibatch < input2_shape.d[N_DIM]; ++ibatch)
	{
		...
		T* im2colWorkspacePtr = static_cast<T*>(workspace) + (maskSize + im2colSize) * ibatch + maskSize;

		// const T* inputMask = static_cast<const T*>(input2) + (ibatch, input2_shape.d[C_DIM] / 3 * 2);
		const int MASK_ELEMS_PER_BATCH = (ibatch*input2_shape.d[C_DIM] + input2_shape.d[C_DIM] / 3 * 2) *
											input2_shape.d[H_DIM] * input2_shape.d[W_DIM];
		const T* inputMask = static_cast<const T*>(input2) + MASK_ELEMS_PER_BATCH;

		dim3 sigmoidGrid(gridDims(maskSize));
		dim3 sigmoidBlock(blockDims(maskSize));
		...
		dim3 im2colGrid(gridDims(im2colSize));
		dim3 im2colBlock(blockDims(im2colSize));
		...
		if (bias)
		{
			...
			dim3 biasGrid(gridDims(edge));
			dim3 biasBlock(blockDims(edge));
